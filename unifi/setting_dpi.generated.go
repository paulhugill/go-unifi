...
func (c *Client) getSettingDpi(ctx context.Context, site string) (*SettingDpi, error) {
	...
	err := c.do(ctx, "GET", fmt.Sprintf("proxy/network/api/s/%s/get/setting/dpi", site), nil, &respBody)
	if err != nil {
		return nil, err
	}
...
func (c *Client) updateSettingDpi(ctx context.Context, site string, d *SettingDpi) (*SettingDpi, error) {
	...
	err := c.do(ctx, "PUT", fmt.Sprintf("proxy/network/api/s/%s/set/setting/dpi", site), d, &respBody)
	if err != nil {
		return nil, err
	}
