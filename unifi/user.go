...
func (c *Client) GetUserByMAC(ctx context.Context, site, mac string) (*User, error) {
	...
	err := c.do(ctx, "GET", fmt.Sprintf("proxy/network/api/s/%s/stat/user/%s", site, mac), nil, &respBody)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateUser(ctx context.Context, site string, d *User) (*User, error) {
	...
	err := c.do(ctx, "POST", fmt.Sprintf("proxy/network/api/s/%s/group/user", site), reqBody, &respBody)
	if err != nil {
		return nil, err
	}
...
func (c *Client) stamgr(ctx context.Context, site, cmd string, data map[string]interface{}) ([]User, error) {
	...
	err := c.do(ctx, "POST", fmt.Sprintf("proxy/network/api/s/%s/cmd/stamgr", site), reqBody, &respBody)
	if err != nil {
		return nil, err
	}
