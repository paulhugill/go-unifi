...
func (c *Client) getSettingUsw(ctx context.Context, site string) (*SettingUsw, error) {
	...
	err := c.do(ctx, "GET", fmt.Sprintf("proxy/network/api/s/%s/get/setting/usw", site), nil, &respBody)
	if err != nil {
		return nil, err
	}
...
func (c *Client) updateSettingUsw(ctx context.Context, site string, d *SettingUsw) (*SettingUsw, error) {
	...
	err := c.do(ctx, "PUT", fmt.Sprintf("proxy/network/api/s/%s/set/setting/usw", site), d, &respBody)
	if err != nil {
		return nil, err
	}
